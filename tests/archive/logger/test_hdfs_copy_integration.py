...
class TestMultipleOperations:
    """Test managing multiple copy operations."""
    ...
    def test_multiple_operations_independence(self, mock_thread, mock_event, log_manager):
        ...
        for name in copy_names:
            log_manager.start_copy(name, [f"/tmp/{name}/*.log"], f"hdfs://dest/{name}/", preserve_structure=False)
    ...
    def test_cleanup_stops_all_operations(self, mock_thread, mock_event, log_manager):
        """Test that LogManager cleanup stops all operations."""
        # Start operations
        log_manager.start_copy("cleanup1", ["/tmp/*.log"], "hdfs://dest1/", preserve_structure=False)
        log_manager.start_copy("cleanup2", ["/var/*.log"], "hdfs://dest2/", preserve_structure=False)

        assert len(log_manager._copy_threads) == 2
...
class TestCleanupIntegration:
    ...
    @patch('utilities.logger.LogManager.stop_all_copy')
    ...
    def test_cleanup_performs_all_operations(self, mock_print, mock_logger_remove, mock_stop_all, mock_atexit, log_manager):
        ...
        # Add a mock thread to trigger the trigger_copy_now call
        mock_thread = MagicMock()
        log_manager._copy_threads["test_op"] = mock_thread
...
class TestConcurrentOperations:
    """Test concurrent operations and their interactions."""
    
    def test_stop_all_with_timeout_failures(self, mock_thread, mock_event, log_manager):
        """Test stop_all_copy when some operations fail to stop within timeout."""
        # Start multiple operations
        for i in range(3):
            log_manager.start_copy(f"op{i}", [f"/tmp/op{i}/*.log"], f"hdfs://dest{i}/", preserve_structure=False)

        # Mock stop_copy to simulate timeout failures for some operations
        original_stop = log_manager.stop_copy
        ...
        with patch.object(log_manager, 'stop_copy', side_effect=mock_stop):
            failed_ops = log_manager.stop_all_copy(timeout=0.1, verbose=True)
    ...
    def test_worker_exception_during_cleanup(self, mock_thread, mock_event, log_manager):
        ...
        # Start an operation
        log_manager.start_copy("error_prone", ["/tmp/*.log"], "hdfs://dest/", preserve_structure=False)
    ...
    def test_concurrent_operations_with_file_overlap(self, mock_thread, mock_event, log_manager):
        ...
        with patch('utilities.logger.LogManager._discover_files_to_copy') as mock_discover:
            ...
            log_manager.start_copy("copy1", ["/tmp/*.log"], "hdfs://dest1/", preserve_structure=False)
            log_manager.start_copy("copy2", ["/var/*.log"], "hdfs://dest2/", preserve_structure=False)
...
class TestSignalIntegration:
    """Test integration between signal handling and operations."""
    
    @patch('utilities.logger.signal.signal')
    @patch('utilities.logger.os.kill')
    def test_signal_cleanup_with_active_operations(self, mock_kill, mock_signal, mock_thread, mock_event, log_manager):
        """Test that signal-triggered cleanup properly handles active operations."""
        # Start copy operations
        log_manager.start_copy("active1", ["/tmp/*.log"], "hdfs://dest1/", preserve_structure=False)
        log_manager.start_copy("active2", ["/var/*.log"], "hdfs://dest2/", preserve_structure=False)
